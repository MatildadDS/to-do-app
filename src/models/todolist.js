...
class ToDoList {
    ...
    static getTasks(id, callback) {
        db.query(`SELECT t.*, d.id as list_id, d.name as list_name FROM todolistapp.to_do d  left join todolistapp.task t on d.id=t.to_do_id where d.id=${id} `, (error, result) => {
            if (error) {
                console.log(error);
                callback(error, null);
                return;
